...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    config_setting = config.get("smartapp_endpoint_secure", False)
    await setup_smartapp_endpoint(hass, config_setting)
    return True
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # Ensure platform modules are loaded ahead of time.
    await async_get_loaded_integration(hass, DOMAIN).async_get_platforms(PLATFORMS)

    remove_entry = False
    try:
        # Check if the app is already setup
        ...
        # Retrieve devices and their status
        ...
        async def retrieve_device_status(device):
            try:
                ...
                _LOGGER.debug(
                    "Unable to update status for device: %s (%s). It will be excluded.",
                    device.label,
                    device.device_id,
                    exc_info=True,
                )
...
async def async_remove_entry(hass: HomeAssistant, entry: ConfigEntry) -> None:
    ...
    try:
        await retry_api_call(api.delete_installed_app, installed_app_id, retries=3)
    except ClientResponseError as ex:
        if ex.status == HTTPStatus.FORBIDDEN:
            _LOGGER.debug(
    ...
    # Remove the app
    try:
        await retry_api_call(api.delete_app, app_id, retries=3)
    except ClientResponseError as ex:
        if ex.status == HTTPStatus.FORBIDDEN:
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                _LOGGER.warning(
                    "API call failed: %s. Retrying in %d seconds (attempt %d/%d)",
                    str(e), delay, attempt + 1, retries,
                )
                asyncio.sleep(delay)
            else:
                _LOGGER.error(
                    "API call failed after %d attempts: %s", retries, str(e)
                )
                raise
...
class DeviceBroker:
    ...
    def __init__(
        ...
        self._assignments = self._assign_capabilities_with_cache(devices)
        self.devices = {device.device_id: device for device in devices}
        self.scenes = {scene.scene_id: scene for scene in scenes}

    @lru_cache(maxsize=128)
    def _assign_capabilities_with_cache(self, devices: Iterable):
        """Assign platforms to capabilities with caching."""
        assignments = {}
    ...
    def connect(self):
        ...
        async def regenerate_refresh_token(now):
            ...
            try:
                ...
                _LOGGER.error(
                    "Error regenerating refresh token for installed app %s: %s",
                    self._installed_app_id,
                    str(e),
                )
